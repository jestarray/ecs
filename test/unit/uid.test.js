import { UIDGenerator, DefaultUIDGenerator, nextSalt, nextGenerator, isSaltedBy } from '../../src/uid';

describe('uid', () => {
  it('should have a default generator', () => {
    expect(DefaultUIDGenerator).to.exist;
  });


  it('should create a new generator', () => {
    let gen = new UIDGenerator();
  ...
  it('should return different sequences with different salts', () => {
    let gen1 = new UIDGenerator(1);
    let gen2 = new UIDGenerator(2);
  ...
  it('should return generator with incremented salts when calling nextGenerator()', () => {
    let gen1 = nextGenerator();
    let gen2 = nextGenerator();
  ...
  describe('isSaltedBy()', () => {
    it('should return true when then id was salted with given salt', () => {
      ...
      expect(isSaltedBy(r1, 1)).to.be.equal(true);
      expect(isSaltedBy(r2, 1)).to.be.equal(false);
      expect(isSaltedBy(r2, 2)).to.be.equal(true);
    });
  });
});
